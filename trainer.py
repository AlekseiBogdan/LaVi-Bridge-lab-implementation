...
def main(args):
    ...
    optimizer = optimizer_class(params_to_optimize, lr=1e-4, betas=(0.9, 0.999), weight_decay=1e-2,
                                eps=1e-8)
    lr_scheduler = get_scheduler("constant", optimizer=optimizer, num_warmup_steps=500,
    ...
    for _ in range(num_train_epochs):
        ...
        for _, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                params_to_clip = (itertools.chain(vis.parameters(), text_encoder.parameters(),
                                                  adapter.parameters()))
